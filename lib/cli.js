...
yargs
  ...
  .option('authorizer.*', {
    description: '<*> configuration of the Authorizer implementation. For detailed info run auth-config --authorizer.type <type>. Type is local file or module to be loaded',
    type: 'string'
  })
  .option('authorizer.type', {
    description: 'Authorizer module to be loaded to aedes environment',
    type: 'string'
  })
  .option('credentials', {
    description: '<file> the file containing the credentials. This option is depraceted and will be removed. Please use authorizer.credentials instead.',
    deprecated: true,
    type: 'string'
  })
  ...
  .command({
    ...
    describe: 'Add a user. Authorizer must support support it'
  })
  .command({
    command: 'rmuser <user>',
    describe: 'Removes a user. Authorizer must support it'
  })
  .command({
    command: ['start', '$0'],
  ...
  .command({
    command: 'auth-config <type>',
    describe: 'Print authorizer options. Build in authorizer types ["./authorizer"]'
  })
  ...
  .example('aedes --authorizer.type "./authorizer" --authorizer.credentials ./credentials.json adduser user1 password1', 'Add/Modify user1 with password1 to credentials')
  .example('aedes --authorizer.type "./authorizer" --authorizer.credentials ./credentials.json rmuser user1', 'Removes user1 from credentials')
  .help()
  .alias('h', 'help')
...
function getConfig (program) {
  ...
  // deprecation of credentials option
...
async function start (program) {
  ...
  const setupAuthorizer = async function () {
    ...
    const authorizer = await authorizerFactory(config.authorizer)
...
async function authorizerFactory (config, force) {
  let Authorizer = null
  ...
  try {
    Authorizer = require(config.type)
  } catch (error) {
    console.log(`Authorizer module ${config.type} was not found. Please chceck module name or install the module.`)
  }
  ...
  // TBD check authorizer implementation init, autheticate, authorizeSubscribe, authorizePublish must be defined
  const authorizer = new Authorizer(config)
  await authorizer.init(force)
...
async function adduser (program) {
  ...
  const authorizer = await authorizerFactory(config.authorizer, true)
  if (!authorizer) {
    throw Error('you must specify a valid authorizer configuration. User config file with --config option or set --authorizer.* values')
  }
...
function authOptions (program) {
  try {
    const Authorizer = require(program.type)
    Authorizer.printOptions()
  } catch (error) {
    console.error('Authorizer not found: ' + program.type)
  }
}
